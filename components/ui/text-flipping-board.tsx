...
const FlapCell = React.memo(function FlapCell({
  ...
  return (
    <div className="flex aspect-[3/5] flex-col overflow-hidden rounded-[2px] border border-neutral-800 md:rounded-[3px] md:border-2">
      {/* Flap content area */}
      <div className="relative flex-1 perspective-dramatic transform-3d">
        <div className="absolute inset-0 z-40 hidden flex-row items-center justify-center md:flex">
          <div className="h-1/2 w-px rounded-tr-sm rounded-br-sm bg-neutral-800" />
          <div className="flex h-px flex-1 bg-neutral-800" />
          <div className="h-1/2 w-px rounded-tl-sm rounded-bl-sm bg-neutral-800" />
        </div>
        ...
        {/* Split line */}
        <div className="pointer-events-none absolute inset-x-0 top-1/2 z-20 h-px -translate-y-[0.5px] bg-neutral-700/50" />
      </div>

      {/* Bottom stripes – decorative, outside the flap area */}
      <div className="h-2 w-full bg-[repeating-linear-gradient(to_bottom,currentColor_0,currentColor_1px,transparent_1px,transparent_0.15rem)] mask-t-from-50% text-neutral-800 opacity-100 md:h-4 md:bg-[repeating-linear-gradient(to_bottom,currentColor_0,currentColor_1px,transparent_1px,transparent_0.2rem)]" />
    </div>
  );
},
(prevProps, nextProps) =>
...
export function TextFlippingBoard({
  ...
  return (
    <div
      className={cn(
        "relative mx-auto w-full max-w-3xl rounded-xl bg-black p-2 shadow-xl md:rounded-2xl md:p-4",
        className,
      )}
    >
      <div
        className="grid gap-[2px] md:gap-[4px] w-full"
        style={{
