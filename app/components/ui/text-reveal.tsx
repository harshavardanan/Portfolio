...
export const TextRevealCard = ({
  ...
  return (
    <div
      ...
      ref={cardRef}
      className={cn(
        "bg-[#1d1c20] w-[40rem] rounded-lg p-8 relative overflow-hidden",
        className
      )}
    >
      {children}
      ...
      <div className="h-40 text-center relative flex items-center overflow-hidden">
        ...
        <div className="overflow-hidden absolute flex justify-center items-center text-center [mask-image:linear-gradient(to_bottom,transparent,white,transparent)] w-full h-full">
    ...
    <div className="absolute inset-0">
      {stars.map((star, i) => (
        <motion.span
          key={`star-${i}`}
          animate={{
            top: `calc(${star.top} + ${randomMove()}px)`,
            left: `calc(${star.left} + ${randomMove()}px)`,
